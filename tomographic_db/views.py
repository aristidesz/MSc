...
from django.http import HttpResponseRedirect
from .forms import FormTomoImages,FormGlobeImages
...
def image(request):
	...
	if form_globe.is_valid():
		...
		def subprocess_cmd(command):
		    process = subprocess.Popen(command.format(REG),executable="/bin/csh",stdout=subprocess.PIPE, shell=True)
		    proc_stdout = process.communicate()[0].strip()
		    print proc_stdout
		subprocess_cmd('''

		set OUTFILEmy = dept-slicesmy; 
		set dx = 0.703125;
		set PROJ = -JKf160/7.0;						
		set REG = -Rg;							
		...
		set i = 1;
		  ...
		  grdimage tmp.grd -C$COL $REG $PROJ -X$tx[$i] -Y$ty[$i] -O -K >> $OUTFILEmy.ps;


		  pscoast $REG $PROJ -Dc -A10000 -W0.4p,dimgrey -O -K >> $OUTFILEmy.ps;


		  psxy {0} $PROJ boundaries.xy -Bg60/g30wsen+ -W0.25p,grey	-O -K >> $OUTFILEmy.ps;
